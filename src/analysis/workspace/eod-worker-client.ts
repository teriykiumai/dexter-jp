...
export function runEodWorker(request: EodWorkerRequest, signal?: AbortSignal): Promise<ObjectRef | null> {
  return new Promise((resolve, reject) => {
    ...
    let done = false;
    const finish = (value: ObjectRef | null, failed = false, identityReview = false) => {
      if (done) return; done = true; clearTimeout(timer); signal?.removeEventListener('abort', abort); worker.terminate();
      if (failed) reject(new WorkspaceError(identityReview ? 'identity_review_required' : 'reference_conflict')); else resolve(value);
    };
    ...
    worker.onmessage = event => {
      try { const data = event.data as { ok: boolean; ref: unknown; code?: string };
        finish(data.ok && data.ref !== null ? parse(ObjectRefSchema, data.ref) : null, !data.ok, data.code === 'identity_review_required');
