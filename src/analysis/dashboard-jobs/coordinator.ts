...
export type DashboardJobKindV1 = 'strategy_validation' | 'technical_refresh' | 'overview_refresh' | 'workspace_catalog' | 'workspace_technical' | 'workspace_supply' | 'workspace_financial' | 'workspace_market_short';
...
export function dashboardCoordinatorFailureV1(error: DashboardJobCoordinatorErrorV1, domain: DashboardJobDomainV1) {
  ...
  if (error.reason === 'active_job_conflict') {
    const labels: Record<DashboardJobKindV1, string> = {
      ...
      workspace_catalog: '銘柄一覧更新', workspace_technical: 'Workspace価格更新', workspace_supply: 'Workspace需給更新', workspace_financial: 'Workspace財務更新',
      workspace_market_short: '市場全体空売り更新',
    };
...
function domainFor(kind: DashboardJobKindV1): DashboardJobDomainV1 {
  if (kind === 'workspace_catalog' || kind === 'workspace_technical' || kind === 'workspace_supply' || kind === 'workspace_financial' || kind === 'workspace_market_short') return 'workspace';
  ...
  async #inventory(): Promise<readonly DashboardJobProjectionV1[]> {
    ...
    for (const domain of this.#adapters.keys()) {
      ...
      for (const job of await adapter.inventory()) {
        ...
        if (job.domain !== domain || !['strategy_validation', 'technical_refresh', 'overview_refresh', 'workspace_catalog', 'workspace_technical', 'workspace_supply', 'workspace_financial', 'workspace_market_short'].includes(job.kind)
