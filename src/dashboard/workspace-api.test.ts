...
test('Workspace guarded API: explicit catalog/EOD, read-only search/open URL, exact chart and corruption isolation', async () => {
  ...
  try {
    ...
    for (const [path, method, allow] of [['jobs', 'GET', 'POST'], ['search', 'POST', 'GET'], ['session', 'DELETE', 'GET'],
      ...
      ['instruments/bad-id', 'POST', 'GET'], ['instruments/bad-id/supply', 'POST', 'GET'], ['instruments/bad-id/financial', 'POST', 'GET'], ['instruments/bad-id/open', 'GET', 'POST'], ['instruments/bad-id/favorite', 'DELETE', 'POST']]) {
    ...
    expect((await call(`instruments/${id}/financial?latest=1`)).status).toBe(400);
    expect((await call(`instruments/${id}/financial`, 'GET', undefined, { host: 'evil.test' })).status).toBe(403);
    expect(WorkspaceResponseSchema.safeParse(await (await call(`instruments/${id}/financial`)).json()).success).toBe(true);
    for (const kind of ['margin', 'issuer_short', 'sector_short', 'financial'])
